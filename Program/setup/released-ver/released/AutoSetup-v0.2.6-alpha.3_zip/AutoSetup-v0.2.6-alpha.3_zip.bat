...
:warpmain
    ...
    @REM         echo.
    @REM         echo -----------------------------------------
    @REM         echo ダウンロードに失敗しました。処理を終了します。
    @REM         echo -----------------------------------------
    @REM         echo.
    @REM         popd
    @REM         pause
    ...
    @REM tarを使ってZIPファイルを解凍
        ...
        @REM @REM installディレクトリが既に存在するか確認
        @REM     if not exist %FILE_N% () else (
        @REM         echo.
        @REM         echo -----------------------------------------
    ...
    @REM     echo.
    @REM     echo -----------------------------------------
    @REM     echo %FILE%を削除しています...
    @REM     echo -----------------------------------------
    @REM     echo.
    @REM     del %FILE%
    @REM         if "!errorlevel!"=="0"(
    @REM             echo 正常に削除が完了しました
    @REM         ) else (
    @REM             echo 削除に失敗しました...
    @REM             echo 処理を終了します。
    @REM             popd
